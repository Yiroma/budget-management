...
public class UserService {
    ...
    public UserResponse updateMe(String email, UserUpdateRequest request) {
        ...
        if (request.email() != null) {
            // TODO : vérifier que l'utilisateur possède le nouvel email (envoi d'un lien de confirmation)
            if (userRepository.existsByEmail(request.email())) {
    ...
    public void deleteMe(String email) {
        // TODO : exiger une confirmation (re-saisie du mot de passe ou confirmation par email) avant suppression
        User user = userRepository.findByEmail(email)
                .orElseThrow(() -> new NotFoundException("Utilisateur introuvable."));
